...
TABLE = 'conveyor_operator_master'
...
def save_data(data):
    ...
    if row_id:
        cur.execute(f"UPDATE {TABLE} SET name=%s, contact_no=%s WHERE id=%s", 
                    [name, contact_no, row_id])
    else:
        cur.execute(f"INSERT INTO {TABLE} (name, contact_no) VALUES (%s, %s) RETURNING id", 
                    [name, contact_no])
